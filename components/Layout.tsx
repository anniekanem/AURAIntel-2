...
import { LayoutDashboard, Radio, Map, FileBarChart2, Bell, Search, User, BarChart3, Database, AlertCircle, Menu, X, Archive, Library } from 'lucide-react';
        ...
        <header className="h-18 md:h-20 border-b border-slate-800 bg-[#0f172a]/90 backdrop-blur-md flex items-center justify-between px-4 md:px-8 z-20 sticky top-0">
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center text-sm md:text-base">
                ...
                <span className="text-white font-medium capitalize text-lg tracking-wide">{currentView.toLowerCase().replace('_', ' ')}</span>
             </div>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="relative hidden md:block">
              <input 
                type="text" 
                placeholder="Search..." 
                className="bg-slate-900 border border-slate-700 rounded-full px-5 py-2.5 pl-11 text-sm text-slate-300 focus:ring-1 focus:ring-cyan-500 outline-none w-48 transition-all focus:w-72 placeholder:text-slate-600"
              />
